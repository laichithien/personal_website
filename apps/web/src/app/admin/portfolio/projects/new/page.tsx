...
import { useCreatePortfolioProject } from "@/hooks/use-admin-api";

export default function NewProjectPage() {
  const router = useRouter();
  const { mutate: createProject, isPending } = useCreatePortfolioProject();
    ...
    createProject(formData, {
      ...
      onError: (err: unknown) => {
        if (err && typeof err === "object" && "response" in err) {
          ...
          setError(
            axiosError.response?.data?.detail || "Failed to create project"
          );
        } else {
    ...
    <div className="max-w-2xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500">Add a new portfolio project</p>
        </div>
      </div>
